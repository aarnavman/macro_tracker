...
def mental_health_support():
    ...
    if sentiment > 0.5:
        # If the sentiment is positive (happy)
        mood = "Happy"
        # Suggest a positive action or recommendation
        st.write("Your mood seems happy! Keep up the positive vibes. 😊")
        st.write("Would you like to try a gratitude practice or a calming meditation?")
        # You can call a function for meditation or gratitude (e.g., gratitude_practice() or calming_meditation())
    elif sentiment < -0.5:
        # If the sentiment is negative (sad)
        mood = "Sad"
        # Suggest a positive action or stress relief activity
        st.write("It seems you're feeling down. 😔 It's okay to have tough days.")
        st.write("Would you like to try a breathing exercise or a mindfulness session?")
        # You can call a function for a breathing exercise or mindfulness session
        # e.g., breathing_exercise() or mindfulness_session()
    else:
        mood = "Neutral"
        # If the mood is neutral
        st.write("Your mood is neutral. Would you like some motivational quotes to brighten your day?")
        # You can call a function for motivational quotes or tips (e.g., motivational_quotes())
...
# Define journal entry input area
    journal_entry = st.text_area("Write your thoughts for today:")

# Check if journal_entry is not empty before analyzing
    if journal_entry:
        mood = analyze_mood(journal_entry)  # Analyze the mood if the entry exists
        st.write(f"Your detected mood is: {mood}")
    else:
        st.write("Please write something in the journal to analyze your mood.")
